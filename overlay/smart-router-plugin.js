...
// Era isso que sumia com os badges Roteador/Modelos no Windows: o plugin
// desativava sozinho no load e a GUI ficava sem o badge (silenciosamente).
// Por isso a resolucao roda dentro do apply() — depois que o boot assentou —
// e, se ainda pegar a corrida, tenta de novo por alguns segundos.
...
module.exports = {
  ...
  async apply(ctx) {
    ...
    if (!core) {
      console.error("[SmartRouter] desativado: nao foi possivel carregar schemastery/dsh-settings.");
      for (const d of resolveDiag) console.error("[SmartRouter]   tentativa: " + d);
      console.error("[SmartRouter] dicas: defina DSH_CLI_LIB=<npm-root>/@deepseek-ai/dsh/lib ou NODE_PATH=<npm-root>/@deepseek-ai/dsh/node_modules");
      return;
    }
    await buildSmartRouter(core.z, core.installSettingsSection).apply(ctx);
  }
};
...
function buildSmartRouter(z, installSettingsSection) {
    ...
    if (webServer) {
      ...
      ctx.on("webserver/index-inject", (table) => {
        table.push({
          kind: "script",
          placement: "body",
          text: `(function(){function openInAppModal(u){var m=document.getElementById("dsh-inapp-modal");if(!m){m=document.createElement("div");m.id="dsh-inapp-modal";var s=m.style;s.cssText="display:none;position:fixed;inset:0;z-index:2147483646;background:rgba(0,0,0,.55);align-items:center;justify-content:center;";var box=document.createElement("div");box.style.cssText="position:relative;width:min(1100px,96vw);height:92vh;background:#0d1117;border:1px solid #444;border-radius:12px;overflow:hidden;";var c=document.createElement("button");c.textContent="X";c.setAttribute("aria-label","Fechar");c.style.cssText="position:absolute;top:8px;right:10px;z-index:2;background:#21262d;color:#e6edf3;border:0;border-radius:50%;width:26px;height:26px;cursor:pointer;font-size:12px;";c.onclick=function(){m.style.display="none";};var f=document.createElement("iframe");f.style.cssText="width:100%;height:100%;border:0;";box.appendChild(c);box.appendChild(f);m.appendChild(box);m.__f=f;m.onclick=function(e){if(e.target===m)m.style.display="none";};document.body.appendChild(m);}m.style.display="flex";m.__f.src=u;}var css="position:fixed;right:16px;bottom:16px;z-index:2147483647;display:inline-flex;align-items:center;gap:6px;background:#4d6bfe;color:#fff;border:0;border-radius:20px;padding:9px 16px;font:13px/1 system-ui,sans-serif;cursor:pointer;box-shadow:0 4px 14px rgba(0,0,0,.5);"; function add(){if(document.getElementById("sr-router-btn"))return;var b=document.createElement("button");b.id="sr-router-btn";b.type="button";b.textContent="\\u26A1 Roteador";b.style.cssText=css;b.onclick=function(){openInAppModal("/smart-router");};(document.body||document.documentElement).appendChild(b);}var n=0;var iv=setInterval(function(){if(document.body)add();if(++n>300)clearInterval(iv);},250);})();`
        });
      });
    }
