...
# Uso: tools/stamp-version.sh   (Vars: DSH_CLONE, DSH_LIVE)
# ═══════════════════════════════════════════════════════════════
set -uo pipefail
...
VER="$(git -C "$CLONE" describe --tags --abbrev=0 2>/dev/null || git -C "$CLONE" rev-parse --short HEAD 2>/dev/null || echo "dev")"
SHA="$(git -C "$CLONE" rev-parse HEAD 2>/dev/null || echo "?")"
